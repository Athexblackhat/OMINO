...
* v9.1 - Updated default aux mode options in default omino.conf
...
* v9.0 - Renamed AUTO_VULNSCAN setting to "VULNSCAN" in omino.conf to perform vulnerability scans via 'normal' mode
* v8.9 - Tuned omino.conf around performance for all scans and recon modes
...
* v8.8 - Added static grep searching rules of all URL's and sub-domains (see omino.conf for details)
* v8.8 - Added verbose status logging to flyover mode showing HTTP status/redirect/title, etc.
* v8.8 - Added integration for Port Scanner Add-on for omino Professional
...
* v8.6 - Added new omino configuration flow that allows persistent user configurations and API key transfer
...
* v8.5 - Removed larger wordlists to reduce install size of omino
...
* v8.4 - Added 8 new omino configuration templates (see /usr/share/omino/conf/)
...
* v8.4 - Disabled legacy Metasploit web exploits (check omino conf to re-enable)
* v8.4 - Fixed issue with dirsearch asterisk being used incorrectly
* v8.4 - Fixed issue with airstrike mode not updated omino Professional v8.0 host list
...
* v8.3 - Added NMAP_OPTIONS setting to omino.conf to configure optional NMap scan settings
* v8.3 - Added option to specify custom omino configuration via (-c) switch
...
* v8.3 - Added flyover mode tuning options to omino.conf
...
* v8.3 - Added omino.conf options for OPENVAS_HOST and OPENVAS_PORT selection for remote instances
* v8.3 - Improved 'vulnscan' mode via OpenVAS to scan the same asset multiple times with improved error handling
* v8.2 - Added root priv check to omino script to run
...
* v8.2 - Added Kali start menu app & icon for omino
* v8.2 - Added check for insecure SSL/TLS connections
* v8.2 - Added NMAP_OPTIONS setting in ~/.omino.conf to configure optional NMap settings
...
* v8.1 - Updated omino Professional autoload settings
...
* v8.1 - Fixed issue with -dh flag to delete host with omino Pro v8.0
...
* v7.1 - Fixed/added Metasploit LHOST/LPORT values to all exploits based on omino.conf settings
...
* v7.0 - Added Slack API notifications (Disabled by default..check ~/.omino.conf)
* v7.0 - Added new command switch to add daily, weekly or monthly omino scheduled scans... check README
...
* v7.0 - Added Gobuster (Disabled by default..check ~/.omino.conf)
...
* v6.2 - Added DockerFile to run omino in Docker (CC. Hariom Vashisth <hariom.devops@gmail.com>)
* v6.2 - Changed option to automatically import all NMap XML's into Metasploit's DB
* v6.2 - Changed option to automatically load omino Professional's report when scans complete
* v6.2 - Added config option to enable/disable subdomain hijacking checks in omino.conf
* v6.2 - Fixed issue with omino --list command having invalid reference
...
* v6.1 - Added configuration options to set LHOST/LPORT for all Metasploit exploits in omino.conf
...
* v6.1 - Added new omino.conf options to enabled/disable all plugins and change settings per user
...
* v6.1 - Fixed issue with omino Professional notepad.html missing
...
* v5.7 - Added omino.conf scan configuration file to customize omino environments by user
...
* v5.6 - Changed automatic report generation to "ON" for omino Pro users
* v5.5 - Added new multi-threaded high speed "flyover" mode added
* v5.5 - Added new scan status mode via (omino --status) command
...
* v5.2 - Added configurable modules/plugins to omino script
...
* v5.1 - Added dnscan to install.sh and updated omino references which were broken
...
* v5.0 - Added omino Pro reporting interface (see https://https://github.com/Athexblackhat for more details)
...
* v4.4 - Fixed issue with omino nuke and airstrike modes not running.
* v4.4 - Added improved SNMP checks via NMap/Metasploit.
* v4.4 - Resolved dependency issue for nfs-common package.
* v4.4 - Fixed bug in omino -fp command switch.
* v4.3 - Fixed bug in version info.
* v4.2 - Fixed bad merge in 4.1 causing omino to break.
* v4.1 - Fixed a few bugs with various command line switches for airstrike and nuke modes.
* v4.1 - Fixed issue with path relative file inclusion via the -f flag. You can now include just the local filename (omino -f targets.txt).
* v4.0 - Added new command switch options for all omino scans (see --help for details)
...
* v3.0 - Improved performance of various omino modes
...
* v2.9 - Created uninstall.sh script to uninstall omino
...
* v2.7 - Fixed issue with omino update command and install.sh not running
* v2.7 - Fixed errors with GooHak
* v2.7 - Fixed syntax errors in omino conditional statements 
...
* v2.5f - Fixes for various bugs reported and fixed by @ifly53e (https://github.com/Athexblackhat/omino/pull/89)
...
* v2.5 - Added HTML report generation via omino 'loot' command
* v2.5 - Added automatic NMap searchsploit integration to find exploits
* v2.5 - Added various improvements to omino discovery scan mode
...
* v1.9 - Added Ubuntu docker image for omino (@menzow)
...
* v1.8c - Added install directory (/usr/share/omino/) to install script for universal access
...
* v1.6 - Added option to set omino full path for universal command line access
...
* v1.5c - Fixed issue which broke link to omino directory
...
* v1.4e - Added auto-upgrade option to install.sh for existing omino installs

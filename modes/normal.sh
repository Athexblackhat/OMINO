...
echo -e "$OKRED _______  __   __  ___   __    _  _______ $RESET"
echo -e "$OKRED |       ||  |_|  ||   | |  |  | ||  _    |$RESET"
echo -e "$OKRED |   _   ||       ||   | |   |_| || | |   |$RESET"
echo -e "$OKRED |  | |  ||       ||   | |       || | |   |$RESET"
echo -e "$OKRED |  |_|  ||       ||   | |  _    || |_|   |$RESET"
echo -e "$OKRED |       || ||_|| ||   | | | |   ||       |$RESET"
echo -e "$OKRED |_______||_|   |_||___| |_|  |__||_______|$RESET"
echo -e "$RESET"
echo -e "$OKORANGE + -- --=[ATHEX BLACK HAT X DIR CYBER"
...
if [[ ${#PORT_CHANGE} -ge 2 ]]; then
  echo "[https://github.com/Athexblackhat] •?((¯°·._.• Port change detected on $TARGET (`date +"%Y-%m-%d %H:%M"`) •._.·°¯))؟•" >> $LOOT_DIR/scans/notifications_new.txt
...
if [[ "$SLACK_NOTIFICATIONS_NMAP_DIFF" == "1" ]] && [[ -s "$LOOT_DIR/nmap/ports-$TARGET.diff" ]]; then
  /bin/bash "$INSTALL_DIR/bin/slack.sh" "[https://github.com/Athexblackhat] •?((¯°·._.• Port change detected on $TARGET (`date +"%Y-%m-%d %H:%M"`) •._.·°¯))؟•"

...
if [[ "$MODE" = "webporthttp" ]]; then
  if [[ "$REPORT" = "1" ]]; then
    ...
    echo "[https://github.com/Athexblackhat] •?((¯°·._.• Started omino scan: http://$TARGET:$PORT [$MODE] (`date +"%Y-%m-%d %H:%M"`) •._.·°¯))؟•" >> $LOOT_DIR/scans/notifications_new.txt
    if [[ "$SLACK_NOTIFICATIONS" == "1" ]]; then
      /bin/bash "$INSTALL_DIR/bin/slack.sh" "[https://github.com/Athexblackhat] •?((¯°·._.• Started omino scan: http://$TARGET:$PORT [$MODE] (`date +"%Y-%m-%d %H:%M"`) •._.·°¯))؟•"
    fi
...
echo -e "$OKGREEN ####### #     # ### #     # ####### "
  ...
  if [[ -z "$port_http" ]]; then
    ...
    if [[ "$JEXBOSS" == "1" ]]; then
      ...
      echo -e "${OKGREEN}====================================================================================${RESET}•x${OKGREEN}[`date +"%Y-%m-%d](%H:%M)"`${RESET}x•"
      cd /tmp/
      python3 /usr/share/omino/plugins/jexboss/jexboss.py -u http://$TARGET:${PORT} | tee $LOOT_DIR/web/jexboss-$TARGET-port${PORT}.raw
    ...
    if [[ "$SMUGGLER" = "1" ]]; then
      ...
      echo -e "${OKGREEN}====================================================================================${RESET}•x${OKGREEN}[`date +"%Y-%m-%d](%H:%M)"`${RESET}x•"
      python3 /usr/share/omino/plugins/smuggler/smuggler.py --no-color -u http://$TARGET:${PORT} | tee $LOOT_DIR/web/smuggler-$TARGET-port${PORT}.txt
    fi
    if [[ "$NUCLEI" = "1" ]]; then
      echo -e "${OKGREEN}====================================================================================${RESET}•x${OKGREEN}[`date +"%Y-%m-%d](%H:%M)"`${RESET}x•"
  ...
  echo "[i] Scanning: http://$TARGET:$PORT/"
    sudo python3 /usr/share/omino/bin/zap-scan.py "http://$TARGET:$PORT/" 
    DATE=$(date +"%Y%m%d%H%M")
  ...
  echo "[https://github.com/Athexblackhat] •?((¯°·._.• Finished omino scan: http://$TARGET:$PORT [$MODE] (`date +"%Y-%m-%d %H:%M"`) •._.·°¯))؟•" >> $LOOT_DIR/scans/notifications_new.txt
  if [[ "$SLACK_NOTIFICATIONS" == "1" ]]; then
    /bin/bash "$INSTALL_DIR/bin/slack.sh" "[https://github.com/Athexblackhat] •?((¯°·._.• Finished omino scan: http://$TARGET:$PORT [$MODE] (`date +"%Y-%m-%d %H:%M"`) •._.·°¯))؟•"
  fi
  if [[ "$LOOT" = "1" ]]; then
    loot
  fi  
